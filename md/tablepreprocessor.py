...
class TablesPreprocessor():
    ...
    class Cell:
        ...
        def __post_init__(self):
            self.content = self.md.convert(self.content)

    def __init__(self, md: Markdown, log_level=0):
        self.md = md
    ...
    def __find_tables(self):
        ...
        # Check for tables at EOF
        if tables and tables[-1].ends_at == -1:
            VERBOSE and print(
                '[Preprocess table]    Finalizing table at EOF (line %s)' % (i - 1))
            tables[-1].ends_at = len(self.lines)
    ...
    def __parse_cell(self, line):
        ...
        cell = TablesPreprocessor.Cell(
            content=content, attributes=cell_attributes, md=self.md)
