...
class GridTableProcessor(markdown.blockprocessors.BlockProcessor):
    ...
    def _render_rows(self, table, parent):
        ...
        if table.has_header:
            ...
            header_subparent = body_subparent = etree.SubElement(
                parent, 'tbody')
    ...
    def _get_all_cells(self, block):
        ...
        table = GridTable(start_row, start_col, len(
            block)-1, len(block[0])-1, header_exists)
    ...
    def _scan_down(self, block, start_row, start_col, cur_col):
        ...
        while start_row + height < len(block):
            ...
            if block[cur_row][cur_col] == '+':
                result = self._scan_left(
                    block, start_row, start_col, cur_col, cur_row)
                if result is None:
                    height += 1
                    continue
                return result
            ...
            if block[check_row][check_col] == '+':
                if start_row == check_row and start_col == check_col:
                    ...
                    cell.text = self._gather_text(
                        block, cell.start_row, cell.start_col, cell.end_row, cell.end_col)
                    return cell
